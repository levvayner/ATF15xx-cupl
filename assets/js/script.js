...
export class PlccChipViewComponent {
    ...
    pinConfiguration = this.icConfigurations[0];
    ...
    //     this.drawDevice();
    // }
    // @ViewChild('ic', {static: true})
    // ic: ElementRef<HTMLCanvasElement> | undefined ;
    ...
    selectPin(event){
        const pin = this.pins.find(p => p.x <= event.pageX  && event.pageX <= p.x + p.w && p.y <= event.pageY  && event.pageY <= p.y + p.h );
        ...
        if(!pin){
            ...
            this.ctx.clearRect(600,0,100,50);
            return;
        }
        ...
        if(this.selectedPin){
            ...
            this.ctx.fillText(`Selected Pin: ${this.selectedPin.id.toFixed(0)}`,600,10,100);
            let typeYCoord = 30;
            this.selectedPin.type.forEach(t => {
                this.ctx.fillText(`${t}`,600,typeYCoord,100);
                typeYCoord+= 10;
            })
        }
    }
    ...
    UpdateDeviceCooridnates(){
        ...
        // this.ic.height = this.height;
        // this.ic.width = this.width;

        const headerHeight = .2*this.height;
        const icRenderPanelHeight = .8*this.height;
...
export class ChipUIPin {x; y; w; h; id; type; orientation;}
