...
export async function registerVariableExtensionProvider(context: vscode.ExtensionContext){

    const variableExtensionProvider = vscode.languages.registerCompletionItemProvider(
        {scheme: 'file', language: 'Cupl'},
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                const triggerChar = document.lineAt(position.line).text.charAt(position.character - 1);
                if( triggerChar !== '.'){
                    return;
                }
                
                // walk through the doc to find if this variable is a pin
                let prefix = document.lineAt(position).text.substring(0, position.character - 1).trim();
                if(prefix.includes(' ')){
                ...
                while(searchPosition.line < position.line || searchPosition.character < position.character){
                let word = document.getWordRangeAtPosition(searchPosition);
                if(!word){
                    const w = nextWord(new vscode.Range(searchPosition, new vscode.Position(searchPosition.line,searchPosition.character + 1)), document.lineAt(searchPosition.line), position);
                    if(!w)
                    {
                    return undefined;
                    }
                ...
                if(document.getText(word).replace(';','') === prefix){
                    ...
                    const firstWord = document.getWordRangeAtPosition(new vscode.Position(line.lineNumber, line.firstNonWhitespaceCharacterIndex));
                    if(document.getText(firstWord).toLowerCase() === 'pin'){
                        //check this pin definition is for our word
                        if( line.text.replace(';','').split(' ').reverse()[0] !== prefix){
                            const w = nextWord(new vscode.Range(searchPosition, new vscode.Position(searchPosition.line,searchPosition.character + 1)), document.lineAt(searchPosition.line), position);
                            if(!w)
                            {
                            return undefined;
                        ...
                        let pinVariable = (name: string) => {
                            let item = new vscode.CompletionItem(name, vscode.CompletionItemKind.Property);
                        ...
                        return makearray('ap', 'ar', 'apmux', 'armux', 'byp', 'ca', 'ce', 'ck', 'ckmux',
                        'd', 'dfb', 'dq', 'imux', 'int', 'io', 'ioar', 'ioap', 'iock', 'iod', 'iol', 
                        'iosp', 'iosr', 'j', 'k', 'l', 'le', 'lemux', 'lfb', 
                        'lq', 'obs', 'oe', 'oemux', 'pr', 'r', 's', 'sp', 'sr',
                        't', 'tec', 'tfb', 't1', 't2');
    ...
    var keywordProvider = vscode.languages.registerCompletionItemProvider(
        {scheme: 'file', language: 'Cupl'},
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                return makearray('PIN','FIELD','PINNODE','MIN','FUSE','APPEND','TABLE', '',
                    'SEQUENCE', 'PRESENT', 'NEXT', 'IF', 'CONDITION', 'FUNCTION', 'TRANSITION' );
            }
        }
    );
    var nextInlineProvider = vscode.languages.registerCompletionItemProvider(
        {scheme: 'file', language: 'Cupl'},
        {
            provideCompletionItems(document: vscode.TextDocument, position: vscode.Position) {
                ...
                const firstWord = document.getText(
                ...
                let item = new vscode.CompletionItem("NEXT", vscode.CompletionItemKind.Keyword);
                    item.range = new vscode.Range(position, position);
                return [item];
            }
        }
