...
export function registerActiveProjectPanelProvider(context: vscode.ExtensionContext) {
    
    providerActiveProject = new ActiveProjectProvider(context.extensionUri);
	context.subscriptions.push(
		vscode.window.registerWebviewViewProvider(ActiveProjectProvider.viewType, providerActiveProject));
	...
	context.subscriptions.push(
        vscode.workspace.onDidOpenTextDocument(providerActiveProject.checkActiveProjectForDocument),
        vscode.workspace.onDidChangeWorkspaceFolders(providerActiveProject.checkActiveProjectForWorkspaceFolder),
        vscode.window.onDidChangeActiveTextEditor(providerActiveProject.checkActiveProjectForTextEditor),
        vscode.window.onDidChangeWindowState(providerActiveProject.checkActiveProjectForWindowState),
	);
...
export class ActiveProjectProvider implements vscode.WebviewViewProvider {
    ...
    public openProjectActiveProject(project: Project | undefined){
        ...
        if(project.devicePins){
            ...
            if(pins)
            {
                ...
                atfOutputChannel.appendLine(`Noproject found for device ${project.device?.pinConfiguration} with ${project.device?.pinCount} pins in a ${project.device?.packageType} package.`);
            }
        }
    }
    ...
    checkActiveProjectForWorkspaceFolder(workspaceFolderEvent: vscode.WorkspaceFoldersChangeEvent) {
        ...
        else if(workspaceFolderEvent.removed && workspaceFolderEvent.removed.length > 0){
	...
	private _getHtmlForWebview(webview: vscode.Webview) {
		...
		const scriptUri = webview.asWebviewUri(vscode.Uri.joinPath(this._extensionUri, 'assets', 'js',  'active-project.js'));
