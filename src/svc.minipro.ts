...
export async function registerMiniProDumpCommand(runMiniProDumpCommandName: string, context: vscode.ExtensionContext){
	const cmdRegisterMiniProDumpHandler = async (treeItem: VSProjectTreeItem | vscode.Uri | undefined) => {
	...
	await context.subscriptions.push(vscode.commands.registerCommand(runMiniProDumpCommandName,cmdRegisterMiniProDumpHandler));
}

export async function registerMiniProEraseCommand(runMiniProEraseCommandName: string, context: vscode.ExtensionContext){
	const cmdRegisterMiniProEraseHandler = async (treeItem: VSProjectTreeItem | vscode.Uri | undefined) => {
		...
		if(!project){
			atfOutputChannel.appendLine(`Failed to erase device. Unable to read project information`);
			return;
		}
		if(!project){
			return;
	...
	await context.subscriptions.push(vscode.commands.registerCommand(runMiniProEraseCommandName,cmdRegisterMiniProEraseHandler));
}
...
export async function runMiniPro(project: Project){	
	try{
		...
		cmdString = `${path.join(miniproPath,'minipro')} ${isWindows() ? '--logicic ' + projectFileProvider.miniproPath + '\logicic.xml --infoic ' + projectFileProvider.miniproPath + '\infoic.xml' : ''} -p "${selectedDeviceName}" -w "${project.jedFilePath.fsPath}"  2>&1 | tee`; 		
...
export async function runMiniProDump(project: Project){	
	try{
        ...
        const dumpFile = path.join( path.dirname(project?.buildFilePath.fsPath) ?? '', fileName);
		...
		//execute		
		atfOutputChannel.appendLine('Downloading using MiniPro ' + project.projectName);		
		cmdString = `${path.join(miniproPath,'minipro')} ${isWindows() ? '--logicic ' + projectFileProvider.miniproPath + '\logicic.xml --infoic ' + projectFileProvider.miniproPath + '\infoic.xml' : ''} -p "${selectedDeviceName /* project.deviceName */}" -r "${dumpFile}"  2>&1 | tee`; 		
		const resp = await command.runCommand('vs-cupl Build', project.projectPath.fsPath, cmdString);
		if(resp.responseCode !== 0){
			atfOutputChannel.appendLine('Error occured calling minipro:' + resp.responseError + ' : ' + resp.responseText);
			vscode.window.setStatusBarMessage('Failed to download ' + project.projectName, 5000);	
			return;
		}
		if(resp.responseText.includes('Warning!')){
			atfOutputChannel.appendLine('Done download using minipro [WITH WARNINGS]:' + resp.responseText.split('\n').filter(l => l.includes('Warning!')).join('\n'));
		}else{
			atfOutputChannel.appendLine('Done download using minipro:' + resp.responseText);
		...
		vscode.window.setStatusBarMessage('Done download ' + project.projectName, 5000);	
		return resp;
...
export async function runMiniProErase(project: Project){
	try{
		...
		let srchString = project.deviceName;
		if(!srchString){
			atfOutputChannel.appendLine('Failed to erase using mini pro. Failed to read device name');
			return;
		}
		let cmdString = `${path.join(miniproPath,'minipro')} -t `; 
		const miniProFound =  await command.runCommand('vs-cupl Build', project.projectPath.fsPath, cmdString);
		...
		if(!selectedDeviceName){
			atfOutputChannel.appendLine('Failed to erase using mini pro. Failed to find device by this name: ' + project.deviceName);
			return;
		}
		...
		//execute		
		atfOutputChannel.appendLine('Erasing using MiniPro ' + project.projectName);		
		cmdString = `${path.join(miniproPath,'minipro')} ${isWindows() ? '--logicic ' + projectFileProvider.miniproPath + '\logicic.xml --infoic ' + projectFileProvider.miniproPath + '\infoic.xml' : ''} -p "${selectedDeviceName /* project.deviceName */}" -E  2>&1 | tee`; 		
		const resp = await command.runCommand('vs-cupl Build', project.projectPath.fsPath, cmdString);
		if(resp.responseCode !== 0){
			atfOutputChannel.appendLine('Error occured calling minipro:' + resp.responseError + ' : ' + resp.responseText);
			vscode.window.setStatusBarMessage('Failed to erase ' + project.projectName, 5000);	
			return;
		}
		if(resp.responseText.includes('Warning!')){
			atfOutputChannel.appendLine('Done erase using minipro [WITH WARNINGS]:' + resp.responseText.split('\n').filter(l => l.includes('Warning!')).join('\n'));
		}else{
			atfOutputChannel.appendLine('Done erase using minipro:' + resp.responseText);			
		}
		
		

		vscode.window.setStatusBarMessage('Done erase ' + project.projectName, 5000);	
		return resp;
